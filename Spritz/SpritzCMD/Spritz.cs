...
namespace SpritzCMD
{
    internal class Spritz
    {
        ...
        private static void Main(string[] args)
        {
            ...
            p.Setup(arg => arg.Vcf)
                .As(SpritzOptionStrings.VcfShort,
            ...
            helpoutro += $"2b) Alternatively, omit both -b and -c to generate a reference proteogenomic database from the Ensembl references." + Environment.NewLine;
            helpoutro += Environment.NewLine;
            p.SetupHelp("h", "help")
            ...
            // A supplied VCF stands in for reads, but only for variant annotation: isoform
            ...
            {
                throw new SpritzException(
                    $"Error: -{SpritzOptionStrings.VcfShort} was specified along with fastqs or SRA " +
                    $"accessions. A supplied VCF replaces variant calling from reads, so pass one or the " +
                    $"other, not both.");
            }
            else if (vcfSpecified && (p.Object.AnalyzeIsoforms || p.Object.Quantify))
            {
                throw new SpritzException(
                    $"Error: -{SpritzOptionStrings.VcfShort} cannot be combined with " +
                    $"-{SpritzOptionStrings.AnalyzeIsoformsShort} or " +
                    $"-{SpritzOptionStrings.QuantifyShort}. Isoform reconstruction and quantification both " +
                    $"need sequencing reads, which a VCF does not provide.");
            }
            else if (vcfSpecified && !p.Object.AnalyzeVariants)
            {
                throw new SpritzException(
                    $"Error: -{SpritzOptionStrings.VcfShort} was specified without " +
                    $"-{SpritzOptionStrings.AnalyzeVariantsShort}, so the VCF would not be used. Add " +
                    $"-{SpritzOptionStrings.AnalyzeVariantsShort} to annotate its variants.");
            }
            else if (analysisSpecified && noSequencesSpecified && !vcfSpecified)
            {
            ...
            else
            {
                ...
                if (vcfSpecified)
                    Console.WriteLine($"NB: Variants will be annotated from {p.Object.Vcf} rather than called from reads.");
